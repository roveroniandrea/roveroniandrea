...
if (!process.argv[2]) {
  ...
  renderToReadme({
    ...
    data: `${dataFormat[0]}${encodeSolution(decodedData.word, decodedData.guessed, decodedData.triesRemaining, decodedData.lettersCalled)}${
      dataFormat[1]
    }`,
    triesRemaining: decodedData.triesRemaining,
    lettersCalled: decodedData.lettersCalled,
    lost: decodedData.triesRemaining <= 0,
    keyboard: renderKeyboard(),
    newGame: newGameUrl(),
  });
}
